...
function ProductInfo({product,onAdd,onBuy}:{product:Product,onAdd:()=>void,onBuy:()=>void}) {
...
function AccountPage({shop,openProduct}:{shop:()=>void;openProduct:(product:Product)=>void}) {
  ...
  return <motion.section className="account-shell" initial={{opacity:0}} animate={{opacity:1}}>
    ...
    {tab==="overview"&&<div className="account-grid"><form className="account-panel profile-form" onSubmit={saveProfile}><p>PROFILE</p><div className="split"><label>First name<input name="firstName" defaultValue={data.customer.firstName}/></label><label>Last name<input name="lastName" defaultValue={data.customer.lastName}/></label></div><label>Registered mobile<input value={data.customer.phone} disabled/></label><small>Mobile-number changes require OTP verification. Contact P&amp;R support to update it.</small><button className="primary">Save profile</button></form><div className="account-panel referral-card"><p>REFER & EARN</p><h2>Give good taste.<br/>Get ₹100.</h2><span>Your friend completes their first paid order. Your reward moves into the wallet automatically.</span><div><code>{data.customer.referralCode}</code><button onClick={copyReferral}><Copy size={15}/> Copy link</button></div></div></div>}
    {tab==="orders"&&<div className="account-list">{data.orders.length?data.orders.map(order=><article key={order.id} className="account-order" onClick={()=>void viewOrder(order.id)}><div><p>ORDER #{order.id}</p><h3>{order.items.map(item=>item.productName).join(", ")||"P&R order"}</h3><span>{new Date(order.createdAt).toLocaleDateString("en-IN")} · {order.status}</span></div><b>₹{order.totalAmount.toLocaleString("en-IN")}</b></article>):<EmptyAccount title="No orders yet." action="Explore the collection" onAction={shop}/>} {orderDetail&&<article className="account-order-detail"><button onClick={()=>setOrderDetail(null)}>Close order details</button><p>ORDER #{String(orderDetail.id)} · {String(orderDetail.status).replaceAll("_"," ")}</p><h3>Tracking</h3><span>{String(orderDetail.courier||"Preparing shipment")}{orderDetail.tracking_id?` · ${String(orderDetail.tracking_id)}`:""}</span><div>{Array.isArray(orderDetail.timeline)&&(orderDetail.timeline as Array<Record<string,unknown>>).map(entry=><p key={String(entry.id)}><b>{String(entry.public_title)}</b><br/>{String(entry.public_description||"")}</p>)}</div>{["pending","awaiting_payment","confirmed","processing"].includes(String(orderDetail.status))&&<button onClick={()=>void cancelOrder(Number(orderDetail.id))}>Cancel this order</button>}<a href={`/api/account/orders/${String(orderDetail.id)}/documents`}>Download invoice PDF</a></article>}</div>}
