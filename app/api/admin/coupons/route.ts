...
const integer = (value: unknown, fallback: number | null = null): number | null => value === "" || value == null ? fallback : Number.isInteger(Number(value)) ? Number(value) : NaN;
const amount = (value: unknown, fallback: number | null = null): number | null => value === "" || value == null ? fallback : Math.round(Number(value) * 100);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!codePattern.test(code) || !["percentage", "fixed"].includes(type) || !Number.isInteger(value) || Number(value) <= 0 || !Number.isInteger(minimumOrder) || Number(minimumOrder) < 0 || !Number.isInteger(perCustomerLimit) || Number(perCustomerLimit) < 1 || (type === "percentage" && Number(value) > 100)) throw new Error("Enter a unique code, a valid discount and valid limits.");
