...
export default function Home() {
  ...
  const [collectionFilter, setCollectionFilter] = useState<"All" | "New" | "Men" | "Women">("All");
  ...
  return <main>
    ...
    <AnimatePresence mode="wait">
      ...
      {view === "collection" && <motion.div key="collection" className="collection-page" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
        <div className="collection-title"><p>COLLECTION / EDITION 001</p><h1>{collectionFilter === "Women" ? "Women" : collectionFilter === "Men" ? "Men" : collectionFilter === "New" ? "New Drop" : "Oversized"}<br/>{collectionFilter === "New" ? "Edition 001" : "Essentials"}</h1><span>{collectionFilter === "Women" ? "Women’s crop tees and everyday essentials." : collectionFilter === "Men" ? "Men’s oversized pieces and daily essentials." : collectionFilter === "New" ? "The latest pieces from P&R." : "Considered pieces in proportion, weight and ease."}</span></div>
        <div className="filter-row"><span>{collectionProducts.length} pieces</span><div><button className={collectionFilter==="All"?"active":""} onClick={() => setCollectionFilter("All")}>All</button><button className={collectionFilter==="New"?"active":""} onClick={() => setCollectionFilter("New")}>New Drop</button><button className={collectionFilter==="Men"?"active":""} onClick={() => setCollectionFilter("Men")}>Men</button><button className={collectionFilter==="Women"?"active":""} onClick={() => setCollectionFilter("Women")}>Women</button><button>Filter <ChevronDown size={13}/></button></div></div>
      ...
      {view === "wishlist" && (
        <AccountPage key="wishlist" initialTab="wishlist" standalone shop={() => goCollection("All")} openProduct={openProduct}/>
      )}
    </AnimatePresence>

    <Footer go={go} goAccount={goAccount} goWishlist={goWishlist}/>
    ...
    <AnimatePresence>{searchOpen && <SearchOverlay products={products} close={() => setSearchOpen(false)} openProduct={openProduct}/>}</AnimatePresence>
...
function AccountPage({shop,openProduct,initialTab="overview",standalone=false}:{shop:()=>void;openProduct:(product:Product)=>void;initialTab?:AccountTab;standalone?:boolean}) {
  ...
  useEffect(()=>{const params=new URLSearchParams(location.search);const referral=params.get("ref");if(referral)localStorage.setItem("pr-referral",referral);const requested=params.get("tab");if(!standalone&&(requested === "orders" || requested === "wishlist" || requested === "addresses" || requested === "wallet"))queueMicrotask(()=>setTab(requested));queueMicrotask(()=>void load())},[standalone]);
  ...
  return <motion.section className="account-shell" initial={{opacity:0}} animate={{opacity:1}}>
    <header className="account-head"><div><p>P&R ACCOUNT</p><h1>{standalone ? "Wishlist" : <>Welcome back,<br/>{data.customer.firstName}.</>}</h1></div></header>
    {!standalone && <nav className="account-tabs">{(["overview","orders","wishlist","addresses","wallet"] as const).map((item)=><button className={tab===item?"active":""} onClick={()=>chooseTab(item)} key={item}>{item === "orders" ? "My orders" : item === "wishlist" ? "Wishlist" : item}</button>)}</nav>}
...
function SearchOverlay({products,close,openProduct}:{products:Product[];close:()=>void;openProduct:(p:Product)=>void}){const [query,setQuery]=useState("");const input=useRef<HTMLInputElement>(null);useEffect(()=>{input.current?.focus();const key=(e:KeyboardEvent)=>e.key==="Escape"&&close();addEventListener("keydown",key);return()=>removeEventListener("keydown",key)},[close]);const tokens=query.toLowerCase().replace(/[^a-z0-9]+/g," ").trim().split(/\s+/).filter(Boolean);const searchText=(p:Product)=>{const edition=Number(p.editionNumber??p.id);return `${p.name} ${p.color} ${p.category} ${p.note||""} edition ${edition} ${String(edition).padStart(3,"0")}`.toLowerCase()};const results=tokens.length?products.filter(p=>tokens.every(token=>searchText(p).includes(token))):products.slice(0,4);return <motion.div className="search-overlay" role="dialog" aria-modal="true" aria-label="Product search" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><header><label htmlFor="site-search">Search P&R</label><input ref={input} id="site-search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search a T-shirt, design or edition"/><button onClick={close} aria-label="Close search"><X/></button></header><div className="search-body"><p>{query ? `${results.length} RESULTS` : "TRENDING NOW"}</p>{results.length?<div>{results.map(p=><button key={p.slug} onClick={()=>{close();openProduct(p)}}><Image src={p.gallery[0].src} alt="" width={90} height={112}/><span><b>{p.name}</b><small>Edition {String(p.editionNumber??p.id).padStart(3,"0")} · {p.color} / {p.category}</small></span><strong>₹{p.price.toLocaleString("en-IN")}</strong></button>)}</div>:<div className="search-empty"><h2>Nothing found.</h2><span>Try a design word, “oversized”, or an edition number.</span></div>}</div></motion.div>}
...
function Footer({go,goAccount,goWishlist}:{go:(v:View)=>void;goAccount:(tab?:AccountTab)=>void;goWishlist:()=>void}){const open=(path:string)=>window.location.assign(path);return <footer className="site-footer"><button className="footer-logo" onClick={()=>go("home")}>P<span>&</span>R</button><div><p>Explore</p><button onClick={()=>go("collection")}>Shop all</button><button onClick={()=>go("collection")}>Men</button><button onClick={()=>go("collection")}>Women</button></div><div><p>Account</p><button onClick={()=>goAccount()}>Profile</button><button onClick={()=>goAccount("orders")}>My orders</button><button onClick={goWishlist}>Wishlist</button><button onClick={()=>goAccount("wallet")}>Wallet</button></div><div><p>Help</p><button onClick={()=>open("/shipping-returns")}>Shipping & Returns</button><button onClick={()=>open("/refund-policy")}>Refund Policy</button><button onClick={()=>open("/size-guide")}>Size Guide</button><button onClick={()=>open("/contact")}>Contact</button></div><div><p>Studio</p><button onClick={()=>open("/instagram")}>Instagram</button><button onClick={()=>open("/privacy")}>Privacy</button><button onClick={()=>open("/terms")}>Terms</button></div><small>© 2026 P&R STUDIOS — INDIA</small></footer>}
