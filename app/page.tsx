...
function Footer({go,goAccount}:{go:(v:View)=>void;goAccount:(tab?:AccountTab)=>void}){const open=(path:string)=>window.location.assign(path);return <footer className="site-footer"><button className="footer-logo" onClick={()=>go("home")}>P<span>&</span>R</button><div><p>Explore</p><button onClick={()=>go("collection")}>Shop all</button><button onClick={()=>go("collection")}>Men</button><button onClick={()=>go("collection")}>Women</button></div><div><p>Account</p><button onClick={()=>goAccount()}>Profile</button><button onClick={()=>goAccount("orders")}>My orders</button><button onClick={()=>goAccount("wishlist")}>Wishlist</button><button onClick={()=>goAccount("wallet")}>Wallet</button></div><div><p>Help</p><button onClick={()=>open("/shipping-returns")}>Shipping & Returns</button><button onClick={()=>open("/refund-policy")}>Refund Policy</button><button onClick={()=>open("/size-guide")}>Size Guide</button><button onClick={()=>open("/contact")}>Contact</button></div><div><p>Studio</p><button onClick={()=>open("/instagram")}>Instagram</button><button onClick={()=>open("/privacy")}>Privacy</button><button onClick={()=>open("/terms")}>Terms</button></div><small>© 2026 P&R STUDIOS — INDIA</small></footer>}
