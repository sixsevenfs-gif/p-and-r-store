...
export async function POST(request: Request) {
  try {
    ...
    if (requestedWalletAmount) {
      ...
      walletAmount = Math.min(requestedWalletAmount, balance, totalAmount);
    }
    const [order] = await db.insert(orders).values({ customerId: customer.id, totalAmount, walletAmount, payableAmount: totalAmount - walletAmount }).returning({ id: orders.id });
    ...
    return Response.json({ orderId: order.id, totalAmount, walletAmount, payableAmount: totalAmount - walletAmount }, { status: 201 });
  } catch {
