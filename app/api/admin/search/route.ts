...
export async function GET(request: Request) {
  ...
  const [products, orders, customers] = await Promise.all([
    ...
    env.DB.prepare("SELECT o.id,o.status,o.total_amount,c.first_name,c.last_name FROM orders o JOIN customers c ON c.id=o.customer_id WHERE CAST(o.id AS TEXT) LIKE ? OR c.email LIKE ? OR c.first_name LIKE ? OR c.last_name LIKE ? ORDER BY o.created_at DESC LIMIT 5").bind(like, like, like, like).all(),
    env.DB.prepare("SELECT id,email,first_name,last_name FROM customers WHERE email LIKE ? OR first_name LIKE ? OR last_name LIKE ? ORDER BY created_at DESC LIMIT 5").bind(like, like, like).all(),
  ]);
